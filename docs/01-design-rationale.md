# Design Rationale — Why JVL is shaped the way it is

This document explains the *why* behind the language. If the [tour](02-language-tour.md)
teaches you to write JVL, this teaches you to argue with its designers.
...
| **Executable program (JVL)** | Assertions, rules, proofs, counterfactuals | Harder to construct correctly |

JVL is the third layer. **It does not replace the other two — it sits on top of
...
> **The graph stores what the case says. The JVL program determines what follows
> from it.**

## 2. The compiler analogy

The cleanest way to see JVL's place: JSON and graphs are like an **AST** — real,
useful structure, but inert. JVL is the **language with semantics** layered over
that structure. An AST node `Assignment(x, 5)` is data; a language says what
running it *does*. JVL says what a legal structure *entails*.
...
So JVL makes status a value in a small lattice
...
This is what lets JVL *tolerate uncertainty instead of hiding it*. You are never
...
JVL keeps every rung of that ladder addressable and traceable.

## 5. Deterministic to parse, easy for an LLM to emit

Two audiences must both read JVL unambiguously: a lawyer and a language model. So
...
pipeline depends on a model being able to produce valid JVL. See [the LLM
kit](../llm/README.md).

## 6. Small core, rich libraries

The language has a handful of keywords. It ships almost no legal vocabulary.
Offences, contract types, and doctrines live in `.jvl` libraries
...
systems combines the three things JVL treats as central:
...
JVL's thesis is that **epistemic status + mandatory provenance + LLM-emittable
...
JVL    = meaning        (typed propositions, rules, provenance)
