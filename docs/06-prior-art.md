# Prior Art — What JVL borrows, and where it differs

JVL is not being invented in a vacuum. Several excellent projects have attacked
neighbouring problems, and the right move is to study them closely and steal
freely. This survey records what each does well and precisely where JVL's goals
diverge. **Before inventing new syntax, read these.**

| Project | What it does | Closest to JVL on |
|---|---|---|
...
| **LegalGraphRAG** | Case graph + retrieval + LLM legal reasoning | the *graph* layer beneath JVL |
...
close to JVL's rule layer. **Where it stops:** L4 is about formalising *rules*.
...
establish something. This is *litigation-shaped*, much closer to JVL's world than
...
### DeonticBench — one exact stage of the JVL pipeline
An LLM translates a legal problem into Prolog, and SWI-Prolog actually runs it.
That `LEGAL TEXT → LLM → PROLOG → SOLVER → ANSWER` loop *is* JVL's core insight,
...
statutes are actually written. JVL wants this; it is on the [roadmap](07-roadmap.md).
...
Alloy, Akoma Ntoso. Its AST and type system are directly relevant to JVL's IR.
...
intermediate layer between text and logic implementations. JVL could plausibly
...
close to how JVL sees its own future backends.
...
record* — is the space JVL is trying to occupy.
...
DeonticBench + L4, and only harden JVL's own syntax around the abstractions those
