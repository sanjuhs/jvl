...
The plan for turning LVL from a working prototype into a full-fledged,
...
*Thesis: an LVL program can replace the JSON/graph layers — it already holds the
facts, relationships, and provenance, and it can compute. Make that real.*

- 🚧 **`lvl emit json`** — deterministic canonical JSON of the whole program
  (parties, atoms, statuses, provenance, rules). The interchange format falls
  out of the program, not a separate extractor.
- 🚧 **`lvl emit graph`** — export the case as a graph (nodes = atoms/parties,
  edges = supports/requires/refutes) in a standard format (JSON graph / DOT).
- ⬜ **`lvl emit` round-trips** — importing the JSON reproduces the same atoms.
...
- 🚧 **`lvl diff a.lvl b.lvl`** — structural + semantic diff: parties, facts
  (amounts/dates), rules, obligations, and resulting statuses added / removed /
  changed. Human-readable and machine-readable output.
- 🚧 **`lvl equiv a.lvl b.lvl`** — semantic-equivalence check: do the two
...
- ⬜ **`lvl assert-same doc.lvl against template.lvl`** — "is this contract the
...
- ✅ **AI helper on the website** — a serverless function drafts an LVL program
  from a plain-English scenario (LLM transforms; the engine decides). Key stays
  server-side; model configurable.
- ✅ **`lvl ask FILE "does B have to repay A?"`** — the LLM maps the English
  question to an LVL query; the engine computes the deterministic answer with its
...
- ✅ **Default logic**: `normally ... except when ...` (Catala-style defeasible
  ...
  example ([05](examples/05-limitation-default.lvl)).
- ✅ **`lvl fmt`** — canonical formatter; round-trip safe (re-parses, idempotent,
...
- ✅ **`lvl check --audit`** — reports any supported conclusion that traces to no
...
- ✅ **JS build of the engine** (`site/lvl-engine.js`) powering an in-browser
  editor with live run — parity-tested against the Python reference.
- ✅ **VS Code / TextMate grammar** (`editor-support/vscode/`) for `.lvl` syntax
  highlighting.
...
- ✅ **Deploy on Vercel** (personal scope) — live at https://lvl-lang.vercel.app;
...
3. Nothing becomes a robot judge; LVL surfaces structure, humans judge.
