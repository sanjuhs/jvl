...
from jvl import compare, serialize
...
def test_emit_json_is_valid_and_has_atoms():
    ev = _eval_file("01-loan-vs-investment.jvl")
...
def test_emit_json_is_deterministic():
    ev1 = _eval_file("02-nda-contract.jvl")
    ev2 = _eval_file("02-nda-contract.jvl")
...
def test_identical_programs_are_equivalent():
    a = _eval_file("04-service-agreement-v1.jvl")
    b = _eval_file("04-service-agreement-v1.jvl")
    assert compare.equiv(a, b).equivalent


def test_reordering_and_comments_do_not_break_equivalence():
    base = (EXAMPLES / "04-service-agreement-v1.jvl").read_text()
...
def test_amended_contract_is_not_equivalent():
    a = _eval_file("04-service-agreement-v1.jvl")
    b = _eval_file("04-service-agreement-v2.jvl")
